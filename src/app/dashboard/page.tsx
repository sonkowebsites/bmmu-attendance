      ...
      <main className="mx-auto max-w-6xl px-5 py-8">
        ...
        <div className="mb-8 grid gap-4 sm:grid-cols-3">
          <div className="card p-5">
            ...
            <p className="mt-2 font-display text-4xl font-semibold">{total}</p>
          </div>
          <div className="card p-5">
            <p className="eyebrow">Added this month</p>
            <p className="mt-2 font-display text-4xl font-semibold">{thisMonth}</p>
          </div>
          <div className="card p-5">
            <p className="eyebrow">Centres reporting</p>
            <p className="mt-2 font-display text-4xl font-semibold">{byCentre.length}</p>
          </div>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <section className="card lg:col-span-2">
            <div className="flex items-center justify-between border-b border-bmmu-black/10 dark:border-bmmu-cream/10 p-5">
              ...
              <Link href="/records" className="text-sm font-medium text-bmmu-green dark:text-bmmu-gold">
                View all →
              </Link>
            </div>
            <ul className="divide-y divide-bmmu-black/5 dark:divide-bmmu-cream/5">
              ...
              {recent.map((r) => (
                <li key={r.id} className="p-5 hover:bg-bmmu-black/5 dark:hover:bg-bmmu-cream/5">
                  <Link href={`/records/${r.id}`} className="flex items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="text-sm text-bmmu-black/60 dark:text-bmmu-cream/60">
                        {r.centre} · {new Date(r.eventDate).toLocaleDateString('en-GB', { dateStyle: 'medium' })} · submitted by {r.submittedBy.name}
                      </p>
                    </div>
          ...
          <section className="card p-5">
            <h2 className="font-display text-lg font-semibold">Records by centre</h2>
