...
export default function NewActivityTypeForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error ?? 'Could not add that activity type.');
      setName('');
      router.refresh();
    } catch (err: any) {
      setError(err.message ?? 'Could not add that activity type.');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-wrap gap-3">
      ...
      <button type="submit" disabled={loading} className="btn-primary">
        {loading ? 'Adding…' : 'Add activity type'}
      </button>
